...
EGLBoolean CreateEGLContext(EGLNativeWindowType hWnd, EGLDisplay *eglDisplay,
  ...
  // Create a surface
  surface =
      eglCreateWindowSurface(display, config, (EGLNativeWindowType)hWnd, NULL);
...
EGLBoolean WinCreate(ESContext *esContext, const char *title) {
  ...
  win = XCreateWindow(x_display, root, 0, 0, esContext->width,
                      esContext->height, 0, CopyFromParent, InputOutput,
...
//  userInterrupt()
//
//      Reads from X11 event loop and interrupt program if there is a keypress,
...
//  esInitContext()
//
//      Initialize ES utility context.  This must be called before calling any
//      other functions.
//
...
void ESUTIL_API esMainLoop(ESContext *esContext) {
  ...
  // Just one iteration! while(userInterrupt(esContext) == GL_FALSE)
  {
    ...
    if (esContext->drawFunc != NULL) esContext->drawFunc(esContext);
...
// esLoadTGA()
//
//    Loads a 24-bit TGA image from a file. This is probably the simplest TGA
//    loader ever. Does not support loading of compressed TGAs nor TGAa with
//    alpha channel. But for the sake of the examples, this is sufficient.
//
