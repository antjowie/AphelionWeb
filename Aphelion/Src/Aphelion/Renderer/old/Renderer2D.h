...
struct Render2DData {
  ...
  Render2DData(const glm::vec2& pos, const glm::vec2& size,
               const glm::vec4& color)
      : Render2DData(pos, size, color, 0) {}
  Render2DData(const glm::vec2& pos, const glm::vec2& size,
               const glm::vec4& color, float radians)
      : pos(pos), size(size), color(color), radians(radians) {}
  Render2DData(const glm::vec2& pos, const glm::vec2& size,
               const ap::TextureRef& texture,
               const glm::vec4& tint = glm::vec4(1))
      : Render2DData(pos, size, texture, 0, tint) {}
  Render2DData(const glm::vec2& pos, const glm::vec2& size,
               const ap::TextureRef& texture, float radians,
               const glm::vec4& tint = glm::vec4(1))
