...
void OpenGLVertexArray::AddVertexBuffer(
  ...
  // Calculate stride
  unsigned stride = 0;
  for (auto elem : buffer->GetElements()) stride += elem.size;
  ...
  for (auto elem : buffer->GetElements()) {
    ...
    glVertexAttribPointer(
        index, elem.GetComponentCount(), BaseTypeToOpenGLType(elem.type),
...
const VertexBufferRef& OpenGLVertexArray::GetVertexBuffer(
